...
export function BarChart({ data, color = "#0F3D2E", color2 = "#6EE7B7", height = 80, showLabels = false }: Props) {
        ...
        return (
          <div key={i} className="flex-1 flex flex-col items-center justify-end">
            <div className="w-full flex flex-col justify-end" style={{ height: chartH }}>
              {v2H > 0 && <div className="w-full rounded-t-[2px]" style={{ height: v2H, background: color2 }} />}
              {v1H > 0 && <div className="w-full" style={{ height: v1H, background: color }} />}
            </div>
            {showLabels && <span className="text-[9px] text-ink-subtle mt-0.5 truncate w-full text-center">{d.label.slice(5)}</span>}
          </div>
        );
      })}
    </div>
