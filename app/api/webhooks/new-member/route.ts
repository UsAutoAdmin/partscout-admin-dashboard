...
/**
 * When `AUTOMATION_EXPECT_INBOUND_RELAY=1` is set (your Mac that runs the job), only requests
 * with matching `x-automation-relay-secret` are accepted. Vercel sets that when `AUTOMATION_RELAY_SECRET`
 * matches. Do not set `AUTOMATION_EXPECT_INBOUND_RELAY` on Vercel. Optional; skip for direct local curl.
 */
...
/**
 * When set (e.g. https://xxx.ngrok-free.app/api/webhooks/new-member), Vercel only verifies
 * the Zapier secret, then POSTs the same body + auth headers to your machine. The Mac must
 * run this app (or handler) and expose it via a tunnel; Vercel does not need Supabase/Gmail/Railway.
 */
  ...
  if (relay) {
    try {
      ...
      return NextResponse.json(
        { error: "Relay to local automation failed", detail: e instanceof Error ? e.message : String(e) },
        { status: 502 },
      );
    }
  }
