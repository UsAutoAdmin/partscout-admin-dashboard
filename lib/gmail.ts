...
export async function sendGmailHtmlEmail(params: {
  ...
  /** Bcc (e.g. admin copy when a member email is sent). */
  bcc?: string;
}): Promise<{ id: string }> {
  const token = await getAccessToken();
  ...
  const bccLine =
    params.bcc?.trim() &&
    // avoid sending a duplicate to the same address as `to`
    params.bcc.trim().toLowerCase() !== params.to.trim().toLowerCase()
      ? `Bcc: ${params.bcc.trim()}`
      : null;
